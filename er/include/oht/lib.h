...
class Oht {
 public:
  ...
  Oht(size_t b, size_t z) : kB_(b), kZ_(z), is_small(true) {}
...
void Oht<Ksize, Vsize>::build(const PrfKey &prf_key, unsigned jobs) {
  if (is_small) {
    std::vector<uint8_t> prf_key_buf(prf_key);
    prf_key_buf.push_back(0);

    prf_key_buf[prf_key_buf.size() - 1] = 1;
    build_pass(bins1_, nullptr, prf_key_buf, jobs);
  } else {
    std::vector<uint8_t> prf_key_buf(prf_key);
    prf_key_buf.push_back(0);
...
bool Oht<Ksize, Vsize>::lookup(Elem<Ksize, Vsize> &query, const PrfKey &prf_key) {
  if (is_small) {
    ...
    prf_key_buf[prf_key_buf.size() - 1] = 1;
    auto bins1_bin_id = PrfInt(prf_key_buf, query.key, Ksize, kB_);
    // for test
    bins1_bin_id = 0;
    const auto bins1_bin_begin = bins1_.begin() + bins1_bin_id * kZ_;
    const auto bins1_bin_end = bins1_bin_begin + kZ_;
    ...
    prf_key_buf[prf_key_buf.size() - 1] = 2;
    auto bins2_bin_id = PrfInt(prf_key_buf, query.key, Ksize, kB_);
    // for test
    bins2_bin_id = 0;
